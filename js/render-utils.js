...
function drawArrow(x0, y0, x1, y1) {
    ...
    const mag = Math.sqrt(Math.pow((x1 - x0), 2) + Math.pow((y1 - y0), 2));
    const arrowHeadLength = Math.min(mag / 3, h / 3);
    const angle = Math.atan2(y1 - y0, x1 - x0);
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x1 - arrowHeadLength * Math.cos(angle - Math.PI / 8), y1 - arrowHeadLength * Math.sin(angle - Math.PI / 8));
    ctx.lineTo(x1 - arrowHeadLength * Math.cos(angle + Math.PI / 8), y1 - arrowHeadLength * Math.sin(angle + Math.PI / 8));
    ctx.fill();
}
...
function drawcell_grayscale(i, j) {
    let r = clamp01(solver.r_dens[ix(i, j, simconfig)]);
    r = Math.floor(255 * r);
...
export function initRenderSettings(canvas, fluidsolver) {
    solver = fluidsolver;
